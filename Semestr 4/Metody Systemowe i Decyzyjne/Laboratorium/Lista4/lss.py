...
def get_polynomial_form(polynomial_degree: int) -> np.ndarray:
    ...
    return np.array([[i] for i in range(0, polynomial_degree + 1)])
...
def print_polynomial(theta: np.ndarray, precission: int = 3) -> str:
    ...
    index = 0
    for coef in theta:
        text_arr += [str(np.around(coef[0], decimals=precission)) + '*x^' + str(index)]
        index += 1
        
    return str.join(' + ', text_arr)
...
def least_squares_solution(X: np.ndarray, Y: np.ndarray, polynomial_degree: int) -> np.ndarray:
    ...
    """
    poly_form = get_polynomial_form(polynomial_degree)
    X_design = np.column_stack([X ** i for i in range(polynomial_degree + 1)])
